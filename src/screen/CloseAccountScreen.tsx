...
export function CloseAccountScreen() {
  ...
  async function getEmptyAccounts() {
    ...
    setEmptyAccounts(myEmptyAccounts);
    setIsFetched(true);
    console.log("my empty accounts", myEmptyAccounts);
  }
  ...
  const CloseAccounts = async (toClose: any) => {
    try {
      if (toClose[0] != undefined) {
        setIsClosing(true);
        setSuccess(false);
        setMessage("");
        const nbPerTx = 5;
